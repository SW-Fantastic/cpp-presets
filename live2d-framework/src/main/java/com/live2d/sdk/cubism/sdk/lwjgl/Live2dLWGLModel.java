...
import com.live2d.sdk.cubism.framework.rendering.lwjgl.CubismRendererLWGL;
...
public class Live2dLWGLModel extends CubismUserModel {
    ...
    public Live2dLWGLModel(Live2dLWGLDelegate delegate) {
        this.configure = delegate.getConfigure();
        if (configure.isMocConsistencyValidation()) {
            mocConsistency = true;
        }
        ...
        this.textureManager = delegate.getTextureManager();
        this.assets = delegate.getAssets();
    ...
    public void loadAssets(String dir, String fileName) {
        if (configure.isDebugLog()) {
            Live2dUtils.printLog("load model setting: " + fileName);
        }

        modelHomeDirectory = dir;
        String filePath = modelHomeDirectory + File.separator + fileName;
        ...
        // Setup renderer.
        CubismRenderer renderer = CubismRendererLWGL.create();
     ...
     */
    public void update() {
        final float deltaTimeSeconds = Live2dUtils.getDeltaTime();
...
//         前回セーブされた状態をロード
        ...
        if (motionManager.isFinished()) {
            startRandomMotion(MotionGroup.IDLE.getId(), Priority.IDLE.getPriority());
        } else {
            // モーションを更新
    ...
    public int startMotion(final String group,
        ...
        if (motion == null) {
            ...
            if (!fileName.equals("")) {
                String path = modelHomeDirectory  + File.separator + fileName;
                ...
                if (motion != null) {
                    ...
                    motionManager.setReservationPriority(Priority.NONE.getPriority());
                    return -1;
                }
            }
        } else {
        ...
        if (debugMode) {
            Live2dUtils.printLog("start motion: " + group + "_" + number);
        }
    ...
    public void draw(CubismMatrix44 matrix) {
        ...
        this.<CubismRendererLWGL>getRenderer().setMvpMatrix(matrix);
        this.<CubismRendererLWGL>getRenderer().drawModel();
    }
    ...
    public void setExpression(final String expressionID) {
        ...
        if (debugMode) {
            Live2dUtils.printLog("expression: " + expressionID);
        }

        if (motion != null) {
            expressionManager.startMotionPriority(motion, Priority.FORCE.getPriority());
        } else {
            if (debugMode) {
                Live2dUtils.printLog("expression " + expressionID + "is null");
            }
        }
    }
    ...
    public CubismOffscreenSurfaceLWGL getRenderingBuffer() {
    ...
    public boolean hasMocConsistencyFromFile(String mocFileName) {
        ...
        path = modelHomeDirectory + File.separator + path;
    ...
    private byte[] createBuffer(final String path) {
        if (configure.isDebugLog()) {
            Live2dUtils.printLog("create buffer: " + path);
        }
        return Live2dUtils.loadFileAsBytes(assets,path);
    }
    ...
    private void setupModel(ICubismModelSetting setting) {
        ...
        // Load Cubism Model
        {
            ...
            if (!fileName.equals("")) {
                ...
                if (configure.isDebugLog()) {
                    Live2dUtils.printLog("create model: " + modelSetting.getModelFileName());
                }
        ...
        // Physics
        {
            ...
            if (!path.equals("")) {
                String modelPath = modelHomeDirectory + File.separator + path;
        ...
        if (modelSetting == null || modelMatrix == null) {
            Live2dUtils.printLog("Failed to setupModel().");
            return;
        }
    ...
    private void preLoadMotionGroup(final String group) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!path.equals("")) {
                ...
                if (debugMode) {
                    Live2dUtils.printLog("load motion: " + path + "==>[" + group + "_" + i + "]");
                }
    ...
    private void setupTextures() {
        for (int modelTextureNumber = 0; modelTextureNumber < modelSetting.getTextureCount(); modelTextureNumber++) {
            ...
            texturePath = modelHomeDirectory  + File.separator + texturePath;

            Live2dLWGLTextureManager.TextureInfo texture = textureManager.createTextureFromPngFile(texturePath);
            final int glTextureNumber = texture.id;

            this.<CubismRendererLWGL>getRenderer().bindTexture(modelTextureNumber, glTextureNumber);

            if (configure.isPreMultipliedAlpha()) {
                this.<CubismRendererLWGL>getRenderer().isPremultipliedAlpha(true);
            } else {
                this.<CubismRendererLWGL>getRenderer().isPremultipliedAlpha(false);
            }
        }
    }
