 ...
 **/
public class CubismOffscreenSurfaceOGL {
     ...
     * Constructor
     */
    public CubismOffscreenSurfaceOGL(GL2 gl2) {
        this.GL2 = gl2;
    }
     ...
     */
    public CubismOffscreenSurfaceOGL(CubismOffscreenSurfaceOGL offscreenSurface) {
    ...
    public void beginDraw(int[] restoreFBO) {
        ...
        if (restoreFBO == null) {
            GL2.glGetIntegerv(GL_FRAMEBUFFER_BINDING, IntBuffer.wrap(oldFBO));
        } else {
            oldFBO = restoreFBO;
        }

        // Set the RenderTexture for the mask to active.
        GL2.glBindFramebuffer(GL_FRAMEBUFFER, renderTexture[0]);
    }
    ...
    public void endDraw() {
        ...
        GL2.glBindFramebuffer(GL_FRAMEBUFFER, oldFBO[0]);
    }
    ...
    public void createOffscreenSurface(final int displayBufferWidth, final int displayBufferHeight, final int[] colorBuffer) {
        ...
        if (colorBuffer == null) {
            ...
            GL2.glGenTextures(1, this.colorBuffer, 0);

            GL2.glBindTexture(GL_TEXTURE_2D, this.colorBuffer[0]);
            GL2.glTexImage2D(
                GL_TEXTURE_2D,
                0,
                GL_RGBA,
        ...
        GL2.glGetIntegerv(GL_FRAMEBUFFER_BINDING, tmpFBO, 0);

        GL2.glGenFramebuffers(1, ret, 0);
        GL2.glBindFramebuffer(GL_FRAMEBUFFER, ret[0]);
        GL2.glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, this.colorBuffer[0], 0);
        GL2.glBindFramebuffer(GL_FRAMEBUFFER, tmpFBO[0]);
    ...
    public void destroyOffscreenSurface() {
        if (!isColorBufferInherited && (colorBuffer != null)) {
            GL2.glDeleteTextures(1, colorBuffer, 0);
            colorBuffer = null;
        }

        if (renderTexture != null) {
            GL2.glDeleteFramebuffers(1, renderTexture, 0);
            renderTexture = null;
        }
    }
