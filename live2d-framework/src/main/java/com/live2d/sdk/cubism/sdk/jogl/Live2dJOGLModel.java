...
import com.live2d.sdk.cubism.framework.rendering.jogl.CubismRendererOGL;
...
public class Live2dJOGLModel extends CubismUserModel {
     ...
     * フレームバッファ以外の描画先
     */
    private final CubismOffscreenSurfaceOGL renderingBuffer;
    ...
    public Live2dJOGLModel(Live2dJOGLDelegate delegate) {
    ...
    public void loadAssets(String dir, String fileName) {
        ...
        // Setup renderer.
        CubismRenderer renderer = CubismRendererOGL.create(gles2);
...
//         前回セーブされた状態をロード
    ...
    public void draw(CubismMatrix44 matrix) {
        ...
        this.<CubismRendererOGL>getRenderer().setMvpMatrix(matrix);
        this.<CubismRendererOGL>getRenderer().drawModel();
    }
    ...
    public CubismOffscreenSurfaceOGL getRenderingBuffer() {
    ...
    private void setupTextures() {
        for (int modelTextureNumber = 0; modelTextureNumber < modelSetting.getTextureCount(); modelTextureNumber++) {
            ...
            Live2dJOGLTextureManager.TextureInfo texture = textureManager.createTextureFromPngFile(texturePath);
            final int glTextureNumber = texture.id;

            this.<CubismRendererOGL>getRenderer().bindTexture(modelTextureNumber, glTextureNumber);

            if (configure.isPreMultipliedAlpha()) {
                this.<CubismRendererOGL>getRenderer().isPremultipliedAlpha(true);
            } else {
