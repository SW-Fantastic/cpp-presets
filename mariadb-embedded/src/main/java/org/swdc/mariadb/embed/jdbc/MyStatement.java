...
public class MyStatement implements Statement {
    ...
    protected List<Long> generateKeys = new ArrayList<>();

    protected MySQLExecutor executor;

    public MyStatement(MySQLExecutor executor, MyConnection connection, MySQLStatement statement,int resultType, int resultConcurrency) {
    ...
    public ResultSet executeQuery(String sql) throws SQLException {
        return executor.execute(db -> {
            ...
            curResultSet = new MyQueryResult(executor,this,rs,resultType);
            if (resultConcurrency == ResultSet.CONCUR_UPDATABLE) {
                throw new SQLFeatureNotSupportedException("does not support updateable now.");
            } else {
                return curResultSet;
            }
        });
